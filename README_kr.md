...
제대로 된 코드를 받아 내는 프롬프트와 그렇지 못한 프롬프트의 차이는 네 가지다.

1. **주의 사항부터 읽히기.** 나머지는 에이전트가 코드에서 유추할 수 있지만, 수명 계약만은
   그럴 수 없다.
2. **바이트 단위로 프로토콜 적기.** "4바이트 길이 프리픽스"는 모호하다. 리틀엔디언인지
   빅엔디언인지, 길이에 헤더가 포함되는지까지 적는다.
3. **빌드가 아니라 증거를 요구하기.** 동시 접속으로 돌려 보게 시킨다. 연결 하나짜리 테스트는
   코드가 틀려도 통과한다.
4. **`SSL0xx` 경고 억제를 금지하기.** 말해 두지 않으면 경고를 만난 에이전트가
   `#pragma warning disable`로 덮을 수 있다.

복사해서 고쳐 쓰면 된다.

**서버를 새로 만들 때**

> SuperSocketLite2로 TCP 게임 서버를 만들어라. 먼저 `Docs/agent/cautions_kr.md`와
> `Docs/agent/recipes_kr.md` § 1을 읽어라.
> 프로토콜은 `[2바이트 전체 길이 LE][2바이트 패킷 ID LE][본문]`이고, 전체 길이에는 헤더 4바이트가
> 포함된다. 최대 패킷 8KB, 최대 접속 3000, 포트 32452.
> 에코 패킷 하나부터 시작해라. 다 되면 서버를 띄우고 `Test/SmokeClient`로 최소 50개 동시 연결에서
> 확인한 뒤에 완료라고 보고해라.

**기존 서버에 패킷 추가**

> `ReqEnterRoom` / `ResEnterRoom` 패킷 쌍을 추가해라. `Protocol.cs`와 `PacketHandlers.cs`의
> 기존 패킷을 그대로 따라가면 된다. 방 상태는 서버의 딕셔너리에 둔다.
> 핸들러를 쓰기 전에 `Docs/agent/cautions_kr.md` 4번을 다시 읽어라 — 요청 본문이 핸들러 밖으로
> 나가면 안 된다. `Test/SmokeClient`로 확인하고 출력을 보여 줘라.

**내장 필터로 안 되는 프로토콜**

> 내 프로토콜은 `[1바이트 타입][3바이트 본문 길이 BE][본문]`이고, 타입 `0x00`은 본문 없는
> 하트비트다. `IRequestInfo`와 수신 필터를 작성해라.
> `Docs/agent/recipes_kr.md` § 2와 § 4를 참고해라. 세션마다 요청 인스턴스를 하나 돌려 써서
> 수신 할당이 0이 되게 하고, 헤더가 파이프 세그먼트에 걸칠 수 있다는 점을 잊지 마라.

**이미 있는 코드 리뷰**

> `GameServer/PacketHandlers.cs`를 `Docs/agent/cautions_kr.md` 끝의 리뷰 체크리스트로 점검해라.
> 특히 부하가 걸려야 터지는 것들이 중요하다 — 요청이나 본문이 핸들러 밖으로 나가는 것,
> 풀 버퍼를 zero-copy로 보내는 것, 시퀀스를 단일 세그먼트로 가정하는 것.
> 고치기 전에 발견한 것부터 보고해라.

**부하에서만 나는 데이터 깨짐 추적**

> 부하가 걸리면 클라이언트가 가끔 다른 요청의 응답을 받는다. 연결 하나로는 재현되지 않는다.
> `Docs/agent/cautions_kr.md` 1·2·4번을 읽고 이 코드가 어느 것을 어겼는지 찾아라.
> 먼저 `Test/SmokeClient -n 100 -c 50 --size 1024 --expect-echo`로 재현부터 하고,
> 고친 뒤 통과하는 것을 보여 줘라.

**패킷을 로직 스레드로 넘기기**

> 지금은 패킷 핸들러가 그 자리에서 처리하고 있다. `Channel`로 먹이는 단일 로직 스레드로 옮겨라.
> `Docs/agent/recipes_kr.md` § 9를 따라라. 핸들러 안에서 `ArrayPool`로 본문을 복사하고,
> 큐가 가득 찬 경로까지 포함해 버퍼 반납은 한 곳에서만 해라.
> 패킷당 할당을 0으로 유지하고, `SSL0xx` 경고는 억제하지 말고 원인을 고쳐라.

위 프롬프트는 `Test/SmokeClient`를 쓰므로 이 저장소를 받아 둔 상태를 전제한다. 패키지만
참조한다면 그 자리에 "`Docs/agent/verify_kr.md`를 따라 검증해라"라고 쓰면 된다. 그 문서에
임시 콘솔 프로젝트에 붙여 넣으면 되는 클라이언트가 통째로 들어 있고,
`dotnet new sslite2-server`로 만든 프로젝트에는 그 문서가 함께 들어간다.
