...
## 2026-08-21 16:24 KST - README 영/한에 에이전트 프롬프트 예시 추가

- `README.md`와 `README_kr.md`의 "AI 코딩 에이전트로 개발하기" 절에 **잘 통하는 프롬프트**
  하위 섹션을 추가했다. 두 판 모두 프롬프트 6개로 개수를 맞췄다.
- 프롬프트 앞에 원칙 4가지를 먼저 적었다. 주의 사항부터 읽히기 / 프로토콜을 바이트 단위로 적기 /
  빌드가 아니라 동시 접속 검증을 요구하기 / `SSL0xx` 경고 억제 금지.
- 예시 6개: 서버 새로 만들기, 기존 서버에 패킷 추가, 내장 필터로 안 되는 프로토콜,
  기존 코드 리뷰, 부하에서만 나는 데이터 깨짐 추적, 패킷을 로직 스레드로 넘기기.
  각 프롬프트가 `Docs/agent/`의 해당 문서와 절 번호를 직접 가리키게 썼다.
- 프롬프트가 `Test/SmokeClient`를 전제하므로, 패키지만 참조하는 경우
  `Docs/agent/verify.md`(한글판 `verify_kr.md`)를 대신 쓰라는 안내를 덧붙였다.
  그 문서에는 단독으로 붙여 넣을 수 있는 클라이언트가 들어 있고, `dotnet new sslite2-server`로
  만든 프로젝트에도 함께 들어간다.
- 문서만 바뀌었다. 링크 검사 깨진 링크 0건.
